...
public class CalculatorApp extends Application {
    ...
    public void start(Stage primaryStage) {
        // Set up the structure of the Calculator Gui, the window name, and the
        // button/field names.
        ...
        for (int i = 0; i < buttonLabels.length; i++) {
            for (int j = 0; j < buttonLabels[i].length; j++) {
                ...
                button.setOnAction(e -> handleButtonClick(label)); // Add action to the button. When the button is
                                                                   // clicked, it will use the label (button name) to
                                                                   // determine action.
    ...
    // The button runs this method will check the input string and run the
    // appropriate method.
    ...
    // This method adds a decimal point to a given number if it does not already
    // exist.
    ...
    public void squareRoot() {
        if (!currentInput.toString().contains("√")) {
            double currentNumber = Double.parseDouble(currentInput.toString()); // Convert the StringBuilder value to a

            // getting the square root of the number
            result = Math.sqrt(currentNumber);

            // clearing output to only display result
            lastNumber = result;
    ...
    // This method handles extraneous circumstances. Perform arithmetic operations
    // if the last operation is not empty or skip otherwise.
    public void handleOperation(String operation) {
        if (!lastOperation.isEmpty()) {
            ...
            lastOperation = operation; // Update the last operation.
            lastNumber = Double.parseDouble(currentInput.toString()); // Convert the currentInput from StringBuilder to
                                                                      // double. Store as lastNumber
            currentInput.setLength(0); // Clear the display field.
        }
    }

    // Perform addition, subtraction, multiplication, or division on the provided
    // numbers based on the operation character.
    public void performOperation() {
        if (!currentInput.toString().isEmpty()) {
            // If the input is not empty, the following code will run.
            double currentNumber = Double.parseDouble(currentInput.toString()); // Convert the StringBuilder value to a
                                                                                // String, then convert from String to
                                                                                // double.
    ...
    public void changeSign() {
        if (!currentInput.toString().isEmpty() && !currentInput.toString().equals("0")) {
            // Only run the following code when the input is not empty and not equal to
            // zero.
            double number = Double.parseDouble(currentInput.toString()); // Convert to double
            number *= -1; // Multiply by -1. Thus, positive numbers become negative and negative numbers
                          // become positive.
    ...
    public void clearLast() {
        if (!currentInput.toString().isEmpty()) {
            ...
            // Otherwise, if the last operation is not empty, set the input to the last
            // number.
            lastOperation = "";
    ...
    public void checkForError() {
        ...
        int containsDecimal = currentInput.indexOf(".") > -1 ? 1 : 0; // If a decimal exists, set this variable to 1,
                                                                      // otherwise set to 0.
        int containsNegativeSign = currentInput.indexOf("-") > -1 ? 1 : 0; // If a negative sign exists, set this
                                                                           // variable to 1, otherwise set to 0.

        // If the output exceeds 8 digits, throw an error. Since the length of the
        // characters in the string are
        // being measured, decimals and negative signs are ignored when determing the
    ...
    public void removeExcessiveDecimalPlaces() {
        // If the result is raised or lowered to a power of 10, first ensure that it
        // does not exceed the digit limit.
